...
import { getMatch, getMe, getTotalAmountByUserId } from "src/_api/api";
...
const VerifPage = () => {
  ...
  const percentage = match?.goalAmount;
  const currentAmount = match?.totalScore || 0;
  ...
  return (
    <div className="bg-gray-5 h-full pb-10">
      ...
      <div className="px-5">
        ...
        <p className="title-md mb-6 pl-[4px]">{user?.name}님의 저축현황</p>
        ...
        {!isVerified && (
          <div className="h-[100px] shadow-[0px_0px_0px_0px_rgba(0,0,0,0),0px_4px_4px_0px_rgba(0,0,0,0)] bg-white rounded-[16px] flex items-center justify-center gap-[27px] mb-4">
            <div className="flex flex-col justify-center items-center gap-[11px]">
              ...
              <p className="title-xs text-purple-30">{totalAmount?.toLocaleString()}원</p>
            </div>
            <Image src={equal} alt="=" />
            <div className="flex flex-col justify-center items-center h-full gap-[2px]">
              <p className="label-md text-gray-80 h-[41%] flex flex-col justify-end">
                아이맥 최신형
              </p>
        ...
        <div className="h-[284px] shadow-[0px_0px_0px_0px_rgba(0,0,0,0),0px_4px_4px_0px_rgba(0,0,0,0)] bg-white rounded-[16px] flex flex-col items-center mb-6 p-6">
          <div className="flex items-center justify-between w-full mb-[40px]">
            ...
            <p className="body-sm text-gray-90">
              목표금액:<span>{match?.goalAmount?.toLocaleString()}</span>원
            </p>
          </div>
          {/* 원형 그래프 */}
          <div className="relative w-[170px] h-[170px]">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              ...
              <p
                id="current-amount"
                className="label-md text-gray-70 text-center"
              >{`${currentAmount.toLocaleString()}원`}</p>
            </div>
        ...
        <div className="w-full mx-auto flex justify-center">
          <Button.Icon onClick={() => router.push("/passbook-verif")} disabled={isVerified}>
            저축 인증하기
          </Button.Icon>
        </div>
        ...
        >
          ...
          <div className="w-[140px] h-[105px]">
            ...
            <p className="caption-md text-gray-70 mb-1">
              목표금액:<span>100,000</span>원
            </p>
            <p className="caption-md text-gray-70 mb-1">
              저축금액:<span>100,000</span>원
            </p>
            <p className="caption-md text-gray-70">
              누적금액:<span>100,000</span>원
            </p>
          </div>
        </div>
      </div>
